...
def history():
    ...
    user_id = session["user_id"]
    transactions = db.execute("SELECT type, symbol, shares, time FROM transactions WHERE user_id = ?", user_id)
...
def register():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return apology("Username has alredy been registerd!")
...
def sell():
    ...
    if request.method == "POST":
        ...
        db.execute("UPDATE users SET cash = ? WHERE id = ?", current_cash + item_price, user_id)
        db.execute("INSERT INTO transactions (user_id, name, shares, price, type, symbol) VALUES (?, ?, ?, ?, ?, ?)",
