...
-- 10:15(a.m.)にてHumphrey Streetのcourthouseにて盗難（アヒルが盗まれる）あり, その際3人の証人にインタビューした(crime_scene_reports)

SELECT transcript FROM interviews WHERE year = 2021 AND month = 7 AND day = 28;
--(Barbara)Leggett Streetにて泥棒がATMでお金を下ろすのを見た(interviews)　泥棒の名前は知らんけど認識はしてた
--(Ruth)10:15から10分以内にパン屋の駐車場から車を盗んで逃走するのを見た(interviews) 探すならその時間帯の逃走車を追うと良いかもね
--(Eugene)泥棒がパン屋を出ようとしたとき, 電話を行った.明日fiftyvilleを出る一番早い便に乗るつもりだと言い, 泥棒は電話の相手に航空券を購入するように頼んだ(interviews)

SELECT license_plate FROM balery_security_logs WHERE year = 2021 AND month = 7 AND day = 28 AND hour = 10 AND minute = 24;
--10:12~10:25までのパン屋警備記録からライセンスプレートを取得, 9個
...
(SELECT destination_airport_idFROM flights WHERE year = 2021 AND month = 7 AND day = 29 AND hour = 8);
--出発時間から算出した朝一番にfiftyvilleを出発する行き先

SELECT passport_number FROM passengers WHERE flight_id
= (SELECT destination_airport_id FROM flights WHERE year = 2021 AND month = 7 AND day = 29 AND hour = 8 AND minute = 20);
--朝一番発の飛行機の乗客パスポート番号の発行

SELECT name, phone_number FROM people WHERE passport_number IN
(SELECT passport_number FROM passengers WHERE flight_id = (SELECT id FROM flights WHERE year = 2021 AND month = 7 AND day = 29 AND hour = 8));
...
(SELECT passport_number FROM passengers WHERE flight_id =
(SELECT id FROM flights WHERE year = 2021 AND month = 7 AND day = 29 AND hour = 8))));
...
--パン屋の記録から怪しい時間内に駐車場からでたものを容疑者候補として算出
